...
- **Question:** EXPERIMENT_QUEUE.md's reframed top row — read the search-behavior object (not solve-count) for the *same* rot180 target across three floor grains (`GEOM` direct primitive → `UNIVERSAL_FLOOR` build_grid+arithmetic → `MINIMAL_COMPLETE_FLOOR` build_grid+read+if+eq, no arithmetic), on both a designed task (3x3 and 10x10) and a real `arc1-train` task (`3c9b0459`). Decomposes **expressible → findable → cheap** on one target.
