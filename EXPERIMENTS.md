...
- **Result:**
  ...
  - **Reusability nuance:** the *higher-order* `mirror_index` has a function-typed arg + closes over `input`, so the current `INT^n→INT`-only `BuildGridSearch` can't compose it as-is; the *refactored first-order* `mirror_index(n,k)` is directly composable (same signature the type-scoping stopgap yields).
...
- **Interpretation:** two independent capabilities, each dissolving the divergence at a different failure point. **(1) Library refactoring** — compress the *library* (walk definitions), not just the corpus — is the piece the current loop lacks and the piece the architecture needs: it *cures the burial* after read-bodies are minted, and works **first-order** (recovers the clean, search-composable `mirror_index(n,k)`). This is the F4 `library refactoring` row, **confirmed** (an earlier draft of this entry wrongly demoted it — corrected). **(2) Higher-order invention** — a distinct, additive Stitch capability — *prevents the burial* on the raw corpus. E8's prediction stands for first-order raw-corpus compression; either capability breaks it. Corollary: the in-house `FrequentSubtree` (first-order, Lam-free, sound Var-holing) would *itself* recover `mirror_index` if simply fed the `.template` bodies and allowed to rewrite definitions — so the refactoring capability isn't intrinsically Stitch-shaped ("feed the proposer the library, rewrite the library"); Stitch is the mature, at-scale, De Bruijn-native implementation of it.
...
## 2026-07-08 — E10: compression↔transfer correlation (the measurement layer's first read)
...
- **Next:** **selection-correct governance** — a usefulness-scored selector vs `GreedyMDL` on E8/E9 (the queued *reusable-idiom governance* row). Standing move unchanged: point the loop at a real ARC slice, where the grade becomes readable at scale.
