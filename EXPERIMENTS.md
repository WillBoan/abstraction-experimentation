...
## 2026-07-12 — perceive→transform: recolor_bg derived from {map_color, most_common_color}
...
- **Design finding en route (worth recording on its own):** the naive corpus design — one example per task, background *and* target color both varying freely — cannot work under the current WAKE mechanics, and this generalizes beyond this experiment. `execute.py` keeps only `ranked_programs[0]`, the *cheapest* solving program per task (`extraction.py` ranks by node count); once any `COLOR` constant source exists, `map_color(g, <literal-bg>, <literal-target>)` (1 application) is always cheaper than the composition through `most_common_color` (2 applications) and always reachable wherever the composition is — so WAKE never even retains the perceiving program, for any per-task variation strategy, as long as a task's goal test is satisfiable by one fixed pair of literals. The fix: a task's goal test is over *all* its train examples at once (`sig == target` on the full training signature, `extraction.py`), so varying the **background across a task's own train examples** makes no single literal source work for that task at all — only `most_common_color(g)`, evaluated per example, can track it. Varying the **target color across different tasks** (fixed within each) then gives antiunify the one differing literal to hole into the free parameter. Background-within/target-across is not symmetric: target varying *within* a task either leaves it unsolved (wasted, if uncorrelated with input) or collapses to a *different*, zero-free-param abstraction (if it does correlate) — never `recolor_bg(g,c)` with `c` free.
...
- **Result:**
  ...
  - **Enablement + full transfer**, shallow (depth-2, one-application) budget: raw `L1` solves **0/5 train, 0/2 eval**; learned `L2` solves **5/5 train, 2/2 eval** — including both held-out, *unseen-during-training* target colors, confirming `c` is genuinely a free parameter, not memorized.
...
- **Interpretation:** multi-generation abstraction — L2 built on a *just-learned* L1, within one recorded LEARN run — works end to end, confirming the wake-sleep loop's iteration structure (not just `GreedyMDLLearnEngine`'s internal corpus-rewrite loop) is what does the work here: `recolor_flipped` was absent from iteration 0's solved corpus entirely, so `abs1` could only be mined after a *second* WAKE search that needed `abs0` to reach its target at all. The considered-count jump (unlike the two single-generation experiments) is the first measured cost signal of a growing library: unlike E1/perceive→transform where the learned primitive strictly *replaces* search depth, `abs0`/`abs1` are additive vocabulary the round-0 leaf set must now carry on every task, including tasks that don't use them.
