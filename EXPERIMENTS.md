...
- **Question:** the design doc deferred a `max_considered` cap ("would make censored baselines and cost-matched controls much cleaner") and early stop. The stronger motivation turned out to be ladder construction: `depth_limit` was doing two conflicting jobs — the ladder's *semantic* validity window AND the compute guard — which is why picking one for a new ladder was guesswork.
...
- **Result:**
  - **Lazy composition is a prerequisite, not an optimization, for early stop.** Solutions are detected *inside* absorption, and a round was fully composed before any of it was absorbed — so on the old structure `solution_limit` could have saved **zero** composition cost in either mode, in the largest round. This was not anticipated in the design doc.
  ...
  - **Per-task, the overshoot is enormous where it exists.** On one solving task (`3c9b0459`, depth_limit 3): 54,447 considered uncapped vs **127** (generation-end) and **18** (immediate), same retained program — the solution sat at candidate index 17. Corpus-level saving is only ~2.7% because just 11/400 tasks solve at all; early stop pays off per *solved* task, not per task.
  - **Certificate hazard found and closed.** `no_skip_paths` / `tractable_jumps` are read off *unsolved* sets, and a censored search is unsolved too — so censoring would have silently converted "we ran out of budget" into a skip-path **PASS**. `no_skip_paths` is now tri-state (`None` = inconclusive) and `None` fails admission exactly as `False` does.
  - **`_estimate_raw_cost` needed the `incomplete` filter.** A truncated final round reads as decay, not growth: unfiltered it returns ~1.1e3 where the truth is 1.1e5 — a plausible-looking number, no error raised. (An earlier guess that it would *invert* the low/high bracket was wrong: `min`/`max` already normalize the ordering. The failure is silent wrongness, not visible inversion.)
- **Interpretation:** ladder construction can now separate the two jobs — set `depth_limit` from the ladder's semantics, cap compute with `considered_limit`, and read "did any search censor?" as "this ladder is less tractable". That is only sound because of the tri-state certificate. Early stop additionally removes the *overshoot* term from §2's laddered-cost breakdown, and its two modes measure §2's two cost-to-first variants directly, rather than as the post-hoc read the 2026-07-17 decision anticipated.
...
- **Next:** decide per ladder whether the reference config should carry a `considered_limit` (it enters `run_id` *and* becomes part of what the certificate is anchored against, so the design doc's depth-only "validity window" would gain a second dimension). Then re-run al1-mirror's raw baseline with a deeper `depth_limit` + `considered_limit` guard — the amortization *considered*-ratio the 2026-07-18 entry left open.
...
- **Interpretation:** **structure is the binding constraint, not learning.** Every rejected ladder except al5/al6/al13 recovered its rungs — al7/al9/al11 recovered 5/5 and were still rejected. The climbs largely worked; the *ladders* were unsound. That inverts the expected failure mode, and it is why the rebuild could be designed on paper (against the collapse families) and certify first time.
...
- **Result:**
  - **The RQ1 raw-cost estimator degrades to noise past ~3 rounds of extrapolation.** Bracket width (hi/lo) vs rounds extrapolated: 2 → 1-10x · 3 → 20-62x · 4 → 250x · 6 → 111-1,125x · 8 → **3.6e7** · 51 (al14) → **7.6e70**. Since design doc §5.3 makes estimation *the method*, this bounds the whole RQ1 programme — and **al2, the designated validation arm, sits at the trivial end** (amortization 1x-1x, raw 31 vs laddered 27), so it cannot validate the regime everything else depends on.
...
- **Result:**
  ...
  - **A finding, not a bug: `warm` is worse than `cold` at depth.** Carrying an early survival rate forward over-predicts, because dedup RISES with pool size (early rounds survive at ~1.0, late rounds far lower). So "probe two rounds and project" is optimistic; the probe now carries the *deepest* observed rate forward and states the residual bias (over-stating cost — the safe direction for a budget).
...
- **Result:**
  - **The product law is exact, not approximate.** Predicted round-1 count == actual, **24/24 cells**. A rung's cost is computable from its floor's signature before anything is built. *[Amended 2026-07-23: the second sentence overstates — round-1 (and any round's count GIVEN its census) is exact arithmetic; whole-cell cost also needs the survival/dedup term, which is semantic and must be measured. The 2026-07-23 validation put one-round-ahead projection at 0.04x–1.24x across floors. The law supports attribution and guards, not cost prediction.]*
...
- **Result:**
  - **Two laws, exact in every measured row.** Generation found = `compositional_depth(p)` (the existing `lam_as_leaf=True`). Minimum `depth_limit` = **effective depth** = `max over frames of (frame depth + descent)`, where each *function-hole fill* opens a frame one `budget.descend()` down. Without lambdas there are no descents and the two collapse to today's single number.
  - **Metrics:**
...
- **Question:** can a floor-lowering claim ("this powerful primitive is really just these simpler ones") be *checked* rather than eyeballed — and does cfb2ce5a's floor actually lower?
...
- **Result:**
  ...
  - **Side finding (tooling defect, fixed):** `lint-ladder` used "has assumed primitives" as its proxy for "is a draft". Implementing the floor falsified that proxy and the command crashed with an unhandled `ValueError`. The two incompletenesses — *assumed floor* and *no train/heldout corpus* — are independent; both now route to the structural tier and report INCOMPLETE.
- **Interpretation:** the loop works and pays immediately, but it **partitions the floor rather than lowering it uniformly**. `retain_colors` was compositional slack and vanished on contact; the perception primitive is irreducible *by construction*, not by accident. That is the cfb2ce5a headline ("the difficulty is perceptual, not compositional") converted from an assertion into a structural fact with a named cause. Lowering that floor is therefore not a rewrite but a deliberate substrate extension — a region-enumeration tier.
...
- **Question:** the promotion loop found `largest_filled_square` irreducible *by construction* (nothing produced a list of regions to select among). Add the missing capability, then check whether the percept -- and the other bespoke cfb2ce5a atoms -- decompose.
- **Ran:** built three base types (`Coord`/`Offset`/`Rect`, `core/geometry.py`) and ~30 primitives (`primitives/addressing.py` + `primitives/regions.py` + `range`/`nth`): region producers (`filled_squares`, `maximal_filled_squares`, `squares`, `quadrants`, `connected_regions`, `content_coords`), the affine algebra (`coord_add`/`coord_sub`/`offset_scale`/`to_local`...), and the missing elim `paste`. Extended `Const` to carry Coord/Offset literals so displacement-typed rungs keep their depth. Refactored the *displacement* family (`translate`/`blank`/`from_cells` -> Offset) + added `extent`. Validated decompositions via the existing `observationally_equivalent_functions` loop.
- **Result:**
  - **`largest_filled_square` is decomposed:** `crop_rect(g, head(filled_squares(g, 0)))`, depth 3, 0 splits. The tripwire test that asserted the substrate *could not* do this fired on the new producer and became the proof it now can -- the whole point of a negative result with a named structural cause.
