...
- **Next:** build cfb2ce5a **v5** with the lowered floor (`filled_squares`/`crop_rect`/`head` in place of `largest_filled_square`), the first taller-than-the-perception ladder; decide the cells-cluster question (raise al14's budget vs redesign vs leave the int/Coord seam); promote the validated decompositions (`largest_filled_square`, `relative_tile`, `retain_colors`) to `analysis/equations.py::BASE_EQUATIONS`.
