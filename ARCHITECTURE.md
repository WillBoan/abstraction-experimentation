...
### 11.3 Short-circuit `If` node · **[proposed — substrate change]**

Add a dedicated AST node `If(cond, then, else)` implementing all `Program` operations, whose `evaluate` computes `cond` and then **only the selected branch** (so a program containing `If` is correct on test inputs, not just during search — §5.4). `result_type` = the (unified) branch type; `children` = `(cond, then, else)`; round-trips via `to_dict`/`from_dict`. The library keeps an `if` **capability entry** `(BOOL, a, a) → a` as the Table-A summoner (§5.4); the enumerator translates it to `If` nodes rather than eager `Apply`. **Substrate change still required:** the eager `IF` primitive currently ships in `control.py` ([control.py:51](src/arc_lab/solvers/program_search/substrate/primitives/control.py#L51)) and must be retired in favour of this node.
