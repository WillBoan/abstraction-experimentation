...
| **low-primitive-floor** | cell floor: `read`, `set_cell`, coord ints (+ λ-index `build_grid` once built) | re-derive D4 members as size-general cell programs | cell-floor basis extensions (sized at design time) | heavy — sized at design time | - **lambda-index binding** (F0, 🔜)<br> - cost-guided **beam** (F1, 🔜) for tractability at scale | The keystone: pixels→D4 tests the whole low-floor thesis. The log's long-standing **[primary open direction]**; queued last only because it's the most gated. |
