...
| **low-primitive-floor** | cell floor: `read`, `set_cell`, `width`/`height`/`sub`, `build_grid` (λ-index) | re-derive D4 members as size-general cell programs | cell-floor basis extensions (sized at design time) | heavy — sized at design time | - bespoke **`build_grid` search** (F1: open-term body enumeration) | The keystone: pixels→D4 tests the whole low-floor thesis. Cost-guided beam (F1) and lambda-index binding (F0, the De Bruijn `Lam`/`Var` substrate) have now both **shipped**; what remains is the *search* that finds `build_grid` programs (open-term body enumeration) — the "where does search break?" probe. The log's long-standing **[primary open direction]**. |
