...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **arc1 preset partition census** | search | `GEOM`=`d4`, `ATOMIC`=`synth`, `SYMMETRY`=`sym` (3 runs) | — (SEARCH: read the partition) | `arc1-train` (400) | presets as-shipped (d4 depth2/arity1/none · synth depth3/arity2/finite · sym depth3/arity4/harvest) | solve count (sanity vs 7/11/10) + `by_category`/`by_provenance` + outcome mix (`DEDUPED`/`PRUNED`/`EVICTED`) per floor; which capability families dominate `considered`; dead primitives | none — runs today (`analyze-run` surfaces the partition) | Zero new code; the partitions are the new signal. Pixel/complete floors (`FLOOR`/`UNIVERSAL_FLOOR`) deliberately excluded — `lambda-synthesis` over 400 real grids is intractable |
