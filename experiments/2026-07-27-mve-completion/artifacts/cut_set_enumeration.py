"""S13: mechanical cut-set enumeration + static all-d2 filter.
...
def affordable(schedule: list[int]) -> bool:
    """Rung-serving levels all d2, top level at most d3 -- see the module docstring (9/9)."""
...
def screen(cohort: Cohort) -> None:
    ...
    print(f"  AFFORDABLE (rungs d2, top <=3): {len(ok)}")
    for selected, schedule, _, status in ok:
        print(f"     {'+'.join(selected):48} {schedule!s:>20}  {status}")
    print(f"  ** NEW and affordable (unexplored, runnable): {len(fresh)}")
    for selected, schedule, _, _ in fresh:
