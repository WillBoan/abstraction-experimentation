...
**The per-check table is generated, not written here** -- [LINT-CHECKS.md](LINT-CHECKS.md) is projected from the checks' own class bodies (`arc-lab lint-checks`), with code, family, stage, severity and run order. The rows below carry the *reasoning* the generated table cannot: why each check exists and what it cannot see.
