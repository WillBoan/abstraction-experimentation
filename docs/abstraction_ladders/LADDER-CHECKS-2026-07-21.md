...
**Where the checks live (2026-07-23).** Each check is a `LadderCheck` subclass in `ladders/checks/`, one module per family below: it declares its `code`, `category`, `stage` and `default_severity` in the class body and implements `run(ctx)`. The lint is the explicit ordered tuple `checks/plan.py::CHECK_PLAN` — order is a value, not import order — run by `checks/run.py::lint_spec` over a `CheckContext` whose derivations are all lazy. `LadderSpec.lint()` is a one-line delegation kept for callers. Adding a check is: write the subclass, add an instance to `CHECK_PLAN`. Nothing else — the skipped-checks list, the tier gating and the finding codes all follow from the class body.
