<!-- Generated by LadderSpec.render() -- regenerate with `arc-lab run-ladder al15-shift-frame --artifacts <dir>`; never hand-edit. -->

# LadderSpec: al15-shift-frame

Derived from `al15-shift-frame.ladder` (in `program_search/ladders/registry/`) -- that file is this ladder's source of truth: floor, rung templates, config and tasks. Everything below is COMPUTED from it.

- Height: 3 (2 bridging rungs + top)
- Floor library: `al15-L0` (3 primitives)
- Depth schedule (`derived`): the `depth_limit` each oracle-chain level runs at, `L_0` first -- [2, 2, 2]. Every sandwich claim below is stated against ITS OWN level's entry.
...
- Static lint (what this file asserts): **OK** -- 61 checks (errors: 0, warnings: 2)
  - warn `not-all-telescope`: every rung has fan-in 1 (a pure telescope)
  - warn `primitive-necessity`: floor primitives carried below the level that first needs them, where the carry is not cheap: pad: needed at L_1 (framed), carried from L_0 -- arity 3, superlinear in the pool at any depth
...
- Dependencies (lower-rung calls, with multiplicity):
  - r_1 `shift_stack`: floor primitives only
  - r_2 `framed`: `shift_stack` x1
  - top `top-00`: `shift_stack` x1, `framed` x1

## Rung spine

| level | rung          | L_i-1 | d_i | needs | d_tmpl | tmpl needs | double-jump | dj needs | fan-in | demos | kind          |
| ----- | ------------- | ----- | --- | ----- | ------ | ---------- | ----------- | -------- | ------ | ----- | ------------- |
| 1     | `shift_stack` | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 0      | 2     | full_solution |
| 2     | `framed`      | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 1      | 2     | full_solution |
...
| ----- | ------------- | --------- | -------- | ----- | ----------------------------------------------------------------------------- |
| 1     | `shift_stack` | 57        | 2        | 28.5x | color: 10 minted / 0 used, int: 4 minted / 0 used, offset: 16 minted / 1 used |
| 2     | `framed`      | 58        | 2        | 29.0x | color: 10 minted / 1 used, int: 4 minted / 1 used, offset: 16 minted / 0 used |
