<!-- Generated by LadderSpec.render() -- regenerate with `arc-lab run-ladder dae9d2b5-recolor-first --artifacts <dir>`; never hand-edit. -->

# LadderSpec: dae9d2b5-recolor-first

Derived from `dae9d2b5-recolor-first.ladder` (in `program_search/ladders/registry/`) -- that file is this ladder's source of truth: floor, rung templates, config and tasks. Everything below is COMPUTED from it.

- Height: 3 (2 bridging rungs + top)
- Floor library: `dae9d2b5-split-L0` (4 primitives)
- Depth schedule (`derived`): the `depth_limit` each oracle-chain level runs at, `L_0` first -- [3, 3, 2]. Every sandwich claim below is stated against ITS OWN level's entry.
...
- Static lint (what this file asserts): **OK** -- 59 checks (errors: 0, warnings: 2)
  ...
  - warn `primitive-necessity`: floor primitives carried below the level that first needs them, where the carry is not cheap: overlay: needed at L_2 (top), carried from L_0 -- depth 3; arity 3, superlinear in the pool at any depth
...
- Dependencies (lower-rung calls, with multiplicity):
  ...
  - r_2 `recolored_east`: floor primitives only
...
| 2     | `recolored_east` | 3     | 3   | 3     | 3      | 3          | 4           | 4        | 0      | 2     | full_solution |
...
| 2     | `recolored_east` | 132       | 5        | 26.4x | color: 10 minted / 2 used, int: 9 minted / 1 used |
...
- `dae9d2b5`: d=2, needs depth_limit 2, over `L_2`
