<!-- Generated by LadderSpec.render() -- regenerate with `arc-lab run-ladder fafffa47-nor-recolor --artifacts <dir>`; never hand-edit. -->

# LadderSpec: fafffa47-nor-recolor

Derived from `fafffa47-nor-recolor.ladder` (in `program_search/ladders/registry/`) -- that file is this ladder's source of truth: floor, rung templates, config and tasks. Everything below is COMPUTED from it.

- Height: 5 (4 bridging rungs + top)
- Floor library: `nor-L0` (5 primitives)
- Depth schedule (`derived`): the `depth_limit` each oracle-chain level runs at, `L_0` first -- [2, 2, 2, 2, 3]. Every sandwich claim below is stated against ITS OWN level's entry.
- Pinned `depth_limit` (what the source file states): 3
- Uniform validity window: `depth_limit` in [3, 2] (inclusive) -- EMPTY, i.e. no single budget serves every level. Not a defect under a derived schedule; that is the constraint per-level budgets remove.
...
- Dependencies (lower-rung calls, with multiplicity):
  ...
  - top `fafffa47`: `recolored_north` x1, `recolored_south` x1
...
| 3     | `recolored_north` | 2     | 2   | 2     | 2      | 2          | 4           | 4        | 1      | 2     | full_solution |
| 4     | `recolored_south` | 2     | 2   | 2     | 2      | 2          | 4           | 4        | 1      | 2     | full_solution |
...
- `fafffa47`: d=3, needs depth_limit 3, over `L_4`
