...
- Depth schedule (`derived`): the `depth_limit` each oracle-chain level runs at, `L_0` first -- [3, 3, 3, 2]. Every sandwich claim below is stated against ITS OWN level's entry.
- Pinned `depth_limit` (what the source file states): 3
- Uniform validity window: `depth_limit` in [3, 5] (inclusive)
- Raw depth profile (top solutions unfolded to `L_0`): [54]

## Verification

- Static lint (what this file asserts): **FAILED** -- 82 checks (errors: 7, warnings: 0)
...
| ----- | -------------- | ----- | --- | ----- | ------ | ---------- | ----------- | -------- | ------ | ----- | ------------- |
| 1     | `move_cell_up` | 3     | 3   | 3     | 3      | 3          | 9           | 9        | 0      | 2     | full_solution |
| 2     | `move_row_up`  | 3     | 3   | 3     | 3      | 3          | 9           | 9        | 3      | 2     | full_solution |
| 3     | `move_grid_up` | 3     | 3   | 3     | 3      | 3          | 6           | 6        | 3      | 2     | full_solution |
...
| ----- | -------------- | --------- | -------- | ------ | ------------------------------------------------- |
| 1     | `move_cell_up` | 300       | 12       | 25.0x  | color: 10 minted / 1 used, int: 5 minted / 2 used |
| 2     | `move_row_up`  | 325       | 9        | 36.1x  | color: 10 minted / 0 used, int: 5 minted / 3 used |
| 3     | `move_grid_up` | 330       | 3        | 110.0x | color: 10 minted / 0 used, int: 5 minted / 3 used |
...
al14-cell-row-grid: FAILED -- 82 checks (7 errors, 0 warnings)
  ERROR constant-subterm[move_cell_up-00]: train-constant subterms beaten by an enumerated literal: sub(1, 1)=0
  ERROR constant-subterm[move_cell_up-01]: train-constant subterms beaten by an enumerated literal: sub(2, 1)=1
  ERROR constant-subterm[move_row_up-00]: train-constant subterms beaten by an enumerated literal: sub(2, 1)=1
  ERROR constant-subterm[move_row_up-01]: train-constant subterms beaten by an enumerated literal: read(set_cell(set_cell(input, sub(3, 1), 0, read(input, 3, 0)), 3, 0, 0), 3, 1)=1, sub(3, 1)=2
