<!-- Generated by LadderSpec.render() -- regenerate with `arc-lab run-ladder 94f9d214-nor-merged --artifacts <dir>`; never hand-edit. -->

# LadderSpec: 94f9d214-nor-merged

Derived from `94f9d214-nor-merged.ladder` (in `program_search/ladders/registry/`) -- that file is this ladder's source of truth: floor, rung templates, config and tasks. Everything below is COMPUTED from it.
...
- Static lint (what this file asserts): **OK** -- 120 checks (errors: 0, warnings: 1)
  - warn `primitive-necessity`: floor primitives carried below the level that first needs them, where the carry is not cheap: overlay: needed at L_4 (merged), carried from L_0 -- arity 3, superlinear in the pool at any depth; map_color: needed at L_2 (recolored_north), carried from L_0 -- arity 3, superlinear in the pool at any depth; swap_colors: needed at L_5 (top), carried from L_0 -- arity 3, superlinear in the pool at any depth
...
- Dependencies (lower-rung calls, with multiplicity):
  - r_1 `north`: floor primitives only
  - r_2 `south`: floor primitives only
  - r_3 `recolored_north`: `north` x1
  - r_4 `recolored_south`: `south` x1
  - r_5 `merged`: `recolored_north` x1, `recolored_south` x1
  - top `94f9d214`: `merged` x1

## Rung spine

| level | rung              | L_i-1 | d_i | needs | d_tmpl | tmpl needs | double-jump | dj needs | fan-in | demos | kind          |
| ----- | ----------------- | ----- | --- | ----- | ------ | ---------- | ----------- | -------- | ------ | ----- | ------------- |
| 1     | `north`           | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 0      | 2     | full_solution |
| 2     | `south`           | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 0      | 2     | full_solution |
| 3     | `recolored_north` | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 1      | 2     | full_solution |
| 4     | `recolored_south` | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 1      | 2     | full_solution |
| 5     | `merged`          | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 2      | 2     | full_solution |
...
- `94f9d214`: d=2, needs depth_limit 2, over `L_5`
