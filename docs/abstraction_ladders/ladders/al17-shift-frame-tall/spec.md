<!-- Generated by LadderSpec.render() -- regenerate with `arc-lab run-ladder al17-shift-frame-tall --artifacts <dir>`; never hand-edit. -->

# LadderSpec: al17-shift-frame-tall

Derived from `al17-shift-frame-tall.ladder` (in `program_search/ladders/registry/`) -- that file is this ladder's source of truth: floor, rung templates, config and tasks. Everything below is COMPUTED from it.

- Height: 4 (3 bridging rungs + top)
- Floor library: `al17-L0` (3 primitives)
- Depth schedule (`derived`): the `depth_limit` each oracle-chain level runs at, `L_0` first -- [2, 2, 2, 2]. Every sandwich claim below is stated against ITS OWN level's entry.
- Pinned `depth_limit` (what the source file states): 2
- Uniform validity window: `depth_limit` in [2, 2] (inclusive)
- Raw depth profile (top solutions unfolded to `L_0`): [6]

## Verification

- Static lint (what this file asserts): **OK** -- 81 checks (errors: 0, warnings: 1)
  - warn `primitive-necessity`: floor primitives carried below the level that first needs them, where the carry is not cheap: pad: needed at L_1 (frame1), carried from L_0 -- arity 3, superlinear in the pool at any depth
...
- Overall: DAG (a rung feeds several or non-adjacent consumers; max fan-out 2), recombination (max fan-in 2)
- Off-spine: none
- Dependencies (lower-rung calls, with multiplicity):
  - r_1 `shift1`: floor primitives only
  - r_2 `frame1`: `shift1` x1
  - r_3 `shift2`: `shift1` x1, `frame1` x1
  - top `top-00`: `shift2` x1

## Rung spine

| level | rung         | L_i-1 | d_i | needs | d_tmpl | tmpl needs | double-jump | dj needs | fan-in | demos | kind          |
| ----- | ------------ | ----- | --- | ----- | ------ | ---------- | ----------- | -------- | ------ | ----- | ------------- |
| 1     | `shift1`     | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 0      | 2     | full_solution |
| 2     | `frame1`     | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 1      | 2     | full_solution |
| 3     | `shift2`     | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 2      | 2     | full_solution |
...
| ----- | -------- | --------- | -------- | ----- | ----------------------------------------------------------------------------- |
| 1     | `shift1` | 57        | 2        | 28.5x | color: 10 minted / 0 used, int: 4 minted / 0 used, offset: 16 minted / 1 used |
| 2     | `frame1` | 58        | 2        | 29.0x | color: 10 minted / 1 used, int: 4 minted / 1 used, offset: 16 minted / 0 used |
| 3     | `shift2` | 59        | 2        | 29.5x | color: 10 minted / 0 used, int: 4 minted / 0 used, offset: 16 minted / 0 used |
