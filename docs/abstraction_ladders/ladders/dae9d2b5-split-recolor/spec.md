<!-- Generated by LadderSpec.render() -- regenerate with `arc-lab run-ladder dae9d2b5-split-recolor --artifacts <dir>`; never hand-edit. -->

# LadderSpec: dae9d2b5-split-recolor

Derived from `dae9d2b5-split-recolor.ladder` (in `program_search/ladders/registry/`) -- that file is this ladder's source of truth: floor, rung templates, config and tasks. Everything below is COMPUTED from it.

- Height: 5 (4 bridging rungs + top)
- Floor library: `dae9d2b5-split-L0` (4 primitives)
- Depth schedule (`derived`): the `depth_limit` each oracle-chain level runs at, `L_0` first -- [2, 2, 2, 2, 2]. Every sandwich claim below is stated against ITS OWN level's entry.
...
- Static lint (what this file asserts): **OK** -- 98 checks (errors: 0, warnings: 1)
...
| 2     | `east`           | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 0      | 2     | full_solution |
| 3     | `recolored_west` | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 1      | 2     | full_solution |
| 4     | `recolored_east` | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 1      | 2     | full_solution |
...
- `dae9d2b5`: d=2, needs depth_limit 2, over `L_4`
