<!-- Generated by LadderSpec.render() -- regenerate with `arc-lab run-ladder al11-greedy-trap --artifacts <dir>`; never hand-edit. -->

# LadderSpec: al11-greedy-trap

Derived from `al11-greedy-trap.ladder` (in `program_search/ladders/registry/`) -- that file is this ladder's source of truth: floor, rung templates, config and tasks. Everything below is COMPUTED from it.
...
- Static lint (what this file asserts): **FAILED** -- 130 checks (errors: 3, warnings: 1)
...
al11-greedy-trap: FAILED -- 130 checks (3 errors, 1 warnings)
  ERROR rewrite-shallow[wide4]: tall4 is reachable over L_2 at depth 2 (<= depth_limit 2) via stack2(stack2(input))
  ERROR rewrite-shallow[tall4]: wide8 is reachable over L_3 at depth 2 (<= depth_limit 2) via stack2(wide4(input))
  ERROR rewrite-shallow[wide8]: top-00 is reachable over L_4 at depth 2 (<= depth_limit 2) via stack2(tall4(input))
  warn  floor-fully-exercised: floor primitives no rung, demonstration, distractor or top solution uses: ['flip_v']
