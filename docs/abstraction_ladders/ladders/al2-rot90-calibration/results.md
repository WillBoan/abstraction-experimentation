...
- Raw (Floor on the top tasks), measured: 7 -- a full-budget FAILURE, not a raw cost (the top is unreachable raw by design); see the estimate below
...
- **Raw cost (estimated): 31 - 31 considered.** Raw is never measured -- for any ladder worth building it is intractable by construction. It is extrapolated from the rounds the Floor search DID complete: observed composed counts [1, 2, 4] through depth 2, projected 2 more rounds to `d_raw`=4 at growth ratios 2.0x (low fit) to 2.0x (high fit).
- **Amortization ratio (estimated): 1x - 1x** against laddered marginal 27. Even the low bracket is the RQ1 answer for this ladder; the spread is method uncertainty, not measurement noise.
