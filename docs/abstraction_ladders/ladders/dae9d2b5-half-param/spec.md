<!-- Generated by LadderSpec.render() -- regenerate with `arc-lab run-ladder dae9d2b5-half-param --artifacts <dir>`; never hand-edit. -->

# LadderSpec: dae9d2b5-half-param

Derived from `dae9d2b5-half-param.ladder` (in `program_search/ladders/registry/`) -- that file is this ladder's source of truth: floor, rung templates, config and tasks. Everything below is COMPUTED from it.
...
- Static lint (what this file asserts): **OK** -- 89 checks (errors: 0, warnings: 2)
  - warn `not-all-telescope`: every rung has fan-in 1 (a pure telescope)
  - warn `primitive-necessity`: floor primitives carried below the level that first needs them, where the carry is not cheap: overlay: needed at L_3 (top), carried from L_0 -- arity 3, superlinear in the pool at any depth; map_color: needed at L_1 (recolored_west), carried from L_0 -- arity 3, superlinear in the pool at any depth
...
- Dependencies (lower-rung calls, with multiplicity):
  - r_1 `half`: floor primitives only
  - r_2 `recolored_west`: `half` x1
  - r_3 `recolored_east`: `half` x1
  - top `dae9d2b5`: `recolored_west` x1, `recolored_east` x1

## Rung spine

| level | rung             | L_i-1 | d_i | needs | d_tmpl | tmpl needs | double-jump | dj needs | fan-in | demos | kind          |
| ----- | ---------------- | ----- | --- | ----- | ------ | ---------- | ----------- | -------- | ------ | ----- | ------------- |
| 1     | `half`           | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 0      | 4     | full_solution |
| 2     | `recolored_west` | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 1      | 2     | full_solution |
| 3     | `recolored_east` | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 1      | 2     | full_solution |
...
| ----- | ---------------- | --------- | -------- | -------- | ------------------------------------------------- |
| 1     | `half`           | 1,211     | 1        | 1,211.0x | color: 10 minted / 0 used, int: 9 minted / 1 used |
| 2     | `recolored_west` | 1,220     | 5        | 244.0x   | color: 10 minted / 2 used, int: 9 minted / 1 used |
| 3     | `recolored_east` | 1,221     | 5        | 244.2x   | color: 10 minted / 2 used, int: 9 minted / 1 used |
...
- `dae9d2b5`: d=2, needs depth_limit 2, over `L_3`
