<!-- Generated by LadderSpec.render() -- regenerate with `arc-lab run-ladder al21-dag-siblings --artifacts <dir>`; never hand-edit. -->

# LadderSpec: al21-dag-siblings

Derived from `al21-dag-siblings.ladder` (in `program_search/ladders/registry/`) -- that file is this ladder's source of truth: floor, rung templates, config and tasks. Everything below is COMPUTED from it.

- Height: 3 (2 bridging rungs + top)
- Floor library: `al21-L0` (4 primitives)
...
- Raw depth profile (top solutions unfolded to `L_0`): [3]

## Verification

- Static lint (what this file asserts): **OK** -- 57 checks (errors: 0, warnings: 1)
...
- Dependencies (lower-rung calls, with multiplicity):
  - r_1 `mirror_h`: floor primitives only
  - r_2 `mirror_flip`: floor primitives only
  - top `top-00`: `mirror_h` x1, `mirror_flip` x1

## Rung spine

| level | rung          | L_i-1 | d_i | needs | d_tmpl | tmpl needs | double-jump | dj needs | fan-in | demos | kind          |
| ----- | ------------- | ----- | --- | ----- | ------ | ---------- | ----------- | -------- | ------ | ----- | ------------- |
| 1     | `mirror_h`    | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 0      | 2     | full_solution |
| 2     | `mirror_flip` | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 0      | 2     | full_solution |
...
| ----- | ------------- | --------- | -------- | ----- | ------- |
| 1     | `mirror_h`    | 4         | 2        | 2.0x  | none    |
| 2     | `mirror_flip` | 5         | 2        | 2.5x  | none    |
...
al21-dag-siblings: OK -- 57 checks (0 errors, 1 warnings)
  warn  not-all-telescope: every rung has fan-in 1 (a pure telescope)
