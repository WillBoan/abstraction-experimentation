<!-- Generated by LadderSpec.render() -- regenerate with `arc-lab run-ladder al20-recolor-telescope --artifacts <dir>`; never hand-edit. -->

# LadderSpec: al20-recolor-telescope

Derived from `al20-recolor-telescope.ladder` (in `program_search/ladders/registry/`) -- that file is this ladder's source of truth: floor, rung templates, config and tasks. Everything below is COMPUTED from it.

- Height: 3 (2 bridging rungs + top)
- Floor library: `al20-L0` (3 primitives)
...
- Static lint (what this file asserts): **OK** -- 61 checks (errors: 0, warnings: 2)
  ...
  - warn `primitive-necessity`: floor primitives carried below the level that first needs them, where the carry is not cheap: map_color: needed at L_1 (recol), carried from L_0 -- arity 3, superlinear in the pool at any depth
...
| ----- | ------------ | ----- | --- | ----- | ------ | ---------- | ----------- | -------- | ------ | ----- | ------------- |
| 1     | `pair`       | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 0      | 2     | full_solution |
| 2     | `recol`      | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 1      | 2     | full_solution |
...
| ----- | ------- | --------- | -------- | ----- | ----------------------------------------------------- |
| 1     | `pair`  | 117       | 2        | 58.5x | color: 10 minted / 0 used, offset: 16 minted / 1 used |
| 2     | `recol` | 118       | 5        | 23.6x | color: 10 minted / 2 used, offset: 16 minted / 0 used |
