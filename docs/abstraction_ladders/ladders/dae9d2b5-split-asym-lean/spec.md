<!-- Generated by LadderSpec.render() -- regenerate with `arc-lab run-ladder dae9d2b5-split-asym-lean --artifacts <dir>`; never hand-edit. -->

# LadderSpec: dae9d2b5-split-asym-lean

Derived from `dae9d2b5-split-asym-lean.ladder` (in `program_search/ladders/registry/`) -- that file is this ladder's source of truth: floor, rung templates, config and tasks. Everything below is COMPUTED from it.

- Height: 4 (3 bridging rungs + top)
- Floor library: `dae9d2b5-split-L0` (4 primitives)
- Depth schedule (`derived`): the `depth_limit` each oracle-chain level runs at, `L_0` first -- [2, 2, 2, 3]. Every sandwich claim below is stated against ITS OWN level's entry.
...
- Static lint (what this file asserts): **OK** -- 78 checks (errors: 0, warnings: 2)
...
- Dependencies (lower-rung calls, with multiplicity):
  - r_1 `west`: floor primitives only
  - r_2 `east`: floor primitives only
  - r_3 `recolored_west`: `west` x1
  - top `dae9d2b5`: `east` x1, `recolored_west` x1

## Rung spine

| level | rung             | L_i-1 | d_i | needs | d_tmpl | tmpl needs | double-jump | dj needs | fan-in | demos | kind          |
| ----- | ---------------- | ----- | --- | ----- | ------ | ---------- | ----------- | -------- | ------ | ----- | ------------- |
| 1     | `west`           | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 0      | 2     | full_solution |
| 2     | `east`           | 2     | 2   | 2     | 2      | 2          | 4           | 4        | 0      | 2     | full_solution |
| 3     | `recolored_west` | 2     | 2   | 2     | 2      | 2          | 3           | 3        | 1      | 2     | full_solution |
...
- `dae9d2b5`: d=3, needs depth_limit 3, over `L_3`
