...
- Search cost metrics (general terms):
  ...
  - **Depth limit (`Budget.depth_limit`)** = the budget's inclusive cap on compositional depth (leaf = 0): a program of depth `d` is reachable iff `d <= depth_limit` — equivalently, the engine runs generations `0..depth_limit` (generation 0 = the round-0 leaves). ONE unit everywhere: program depth, solve generation, and the budget cap all speak it; the engine's internal round count (`depth_limit + 1`) is the only place a `+1` exists.
    - **Corrected (2026-07-22) — the unity holds for FIRST-ORDER programs only.** Measured against the engine: a `Lam` is not a leaf (it is composed into the pool at a round >= 1 and consumed the round after, so it costs one generation), and a lambda body is built in a sub-search that `budget.descend()` hands `depth_limit - 1`. So for a higher-order program the three quantities separate: **solve generation** = `compositional_depth`, while **the budget cap** needs `min_depth_limit` = `max over frames of (frame depth + descent)`. A program of generation 2 can require `depth_limit` 4. Both live in `analysis/depth.py`; every ladder in the batch is first-order, where they coincide and this paragraph's original claim stands.
...
> **Settled (2026-07-19).** Estimation is the METHOD, not a fallback. Raw cost is **never measured** for a real ladder — a ladder worth building is one whose raw search is intractable by construction, so "just run it deeper" does not scale and is not the plan. The **extrapolated baseline** below is implemented (`ladders/report.py::_estimate_raw_cost`): fit the per-round `composed` growth over the rounds the Floor search did complete, project the missing `d_raw - depth_limit` rounds, and report a bracket (geometric-mean fit as the low end, last-observed ratio as the high end — growth accelerates early and decays late as dedup rises, so the truth is between). It runs on every ladder for free, off atoms already recorded. The one deliberate exception is a **calibration ladder** (height 2, tiny floor), where raw IS cheap to measure — its purpose is to _validate the estimator_, which every other ladder's RQ1 number then depends on. Standing caveat: the estimate assumes a pool large enough not to bind; a `max_pool`-capped deep run is cheaper and simply fails, so raising `depth_limit` without raising `max_pool` measures pool starvation, not depth cost.
...
9. **Budget stop limits — BUILT (2026-07-20).** Shipped as `Budget.considered_limit` / `Budget.solution_limit` (the names `max_considered` / `stop_after_solutions` used earlier in this doc), each with a `*_mode` of `immediate` or `generation-end`. They are params (§3.2), not deferred machinery. Three consequences for this design:
   - **`depth_limit` is no longer doing double duty.** It can be set purely from the ladder's semantics (the validity window, §3.1) while `considered_limit` carries the compute guard. The construction loop becomes: run the ladder at the semantically-correct depth, and read _"did any search censor?"_ as "this ladder is less tractable" — a uniform predicate replacing the guesswork in §5.1's "should be tractable, with headroom".
   - **A censored cell is INCONCLUSIVE, never a pass.** The certificate's checks (§6.5) are read off _unsolved_ sets, and a censored search is unsolved too — so `no_skip_paths` is tri-state and `None` fails admission exactly as `False` does. Without this, censoring would silently certify a ladder whose skip path the search merely never reached.
