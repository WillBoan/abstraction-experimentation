...
- **Do not write measured costs into `.ladder` files.** LADDER-FORMAT §1 keeps chosen/derived separate; a stale cost comment is worse than none. Instead have `lint-ladder`/`probe-ladder` **print recorded numbers alongside the rung table**, read from the artifact — co-location without a second source of truth. **Exception:** a retired dead-end draft carries its verdict in its header, since the verdict is the reason the file exists (as the three recovered drafts already do).
...
2. **The ladder development loop** — task selection -> floor design -> spine design -> demo plan -> lint -> probe -> taskgen -> run -> analyse, with a **decision table mapping each failure mode to its response** (the thing improvised throughout 2026-07-25).
...
- **MVE-PLAN-2026-07-25.md** — screen leg 3 currently encodes depth as the filter; the 2026-07-25 data says breadth is the variable that mattered. Refresh the stale "no real-task ladder lints clean" table.

## Verification

1. `make check` green (ruff · mypy --strict · full pytest incl. slow locks). Baseline: 921 passed.
2. **A1 is the load-bearing check:** `uv run arc-lab run-ladder dae9d2b5-halves-union` must reach a certificate with **non-zero demonstration health on rung 2** (`east`), which the current code scores 0.0. This is the end-to-end proof the DAG sweep worked.
3. `lint-ladder` on the four registry/draft ladders — `dae9d2b5-halves-union` (clean), `dae9d2b5-1-arith-halves`, `dae9d2b5-2-merge-rung`, `4347f46a-1-shift-rungs` — verifying the new depth/breadth columns render and the breadth ratio flags `dae9d2b5-1-arith-halves`'s floor.
4. **Per-rung budgets:** confirm the natural 4-rung `dae9d2b5` (`dae9d2b5-3-recolor-rungs`: west/east/recolored_west/recolored_east) lints clean, which the global window rejects today.
   - ~~re-lint `dae9d2b5-1-arith-halves`, whose global window is degenerate `[4,2]`; per-rung it should become valid~~ — **wrong, corrected 2026-07-26.** Three of that draft's rungs have `jump_needs == double_jump_needs` (a deep fragment wrapper inflates the rung's own cost to meet its consumer's inlined cost), and no budget schedule separates those. It is a retired draft, so nothing is lost; the right subject is `dae9d2b5-3-recolor-rungs`, which does become valid (`[3,3,2,2,2]`, 0 errors).
5. **Probe:** two cells per rung on `dae9d2b5-halves-union` at the fixed 50k guard, completing in seconds, with `by_primitive` shown and the pruned/full ratio reported (expect ~10^6 on rung 1). Cells appear in `runs/` and a re-run is a cache hit.
6. **Probe/certificate agreement** — with probe cells now recorded, compare the probe's per-rung verdicts against the certificate's for `dae9d2b5-halves-union`. This is the seam check that has been unmaintained since 2026-07-22; worth landing as a test now that both verdicts exist for one ladder.
7. Regression locks unmoved; any deliberate movement updated with rationale.
