...
Everything a file answers on its own. A rule belongs here only if it needs nothing but the parsed document; the rest of the load-time errors are raised while _constructing_ a library, template or config, so they have no completed model to be a predicate over.
...
| 13 | `top-double-jump-intractable` | structural | error | No top solution is reachable over L\_{k-1} (with the top rung skipped). |
...
S1. `floor-non-empty` S2. `floor-names-unique` S3. `config-paths-unique` S4. `task-ids-unique`
