...
A dated snapshot of every check a Ladder passes through, taken after the `.ladder` migration and the demonstration-plan checks landed; updated 2026-07-22 when the evaluation-backed checks (`constant-subterm`, `if-condition-varies`) and the equational `rewrite-shallow` check landed. Point-in-time: the living sources are [LADDER-FORMAT.md](../abstraction_ladders/LADDER-FORMAT.md) (the format's normative rules) and the code itself (`ladders/lang/`, `ladders/checks/` — one `LadderCheck` subclass per check, ordered by `checks/plan.py::CHECK_PLAN` — `analysis/equations.py` + `analysis/rewrite.py`, `ladders/certificate.py`). The planned-checks list this is measured against is [ABSTRACTION-LADDERS-2026-07-16.md](../abstraction_ladders/ABSTRACTION-LADDERS-2026-07-16.md) §6.4.
...
**The per-check table is generated, not written here** -- [LINT-CHECKS.md](../abstraction_ladders/LINT-CHECKS.md) is projected from the checks' own class bodies (`arc-lab lint-checks`), with code, family, stage, severity and run order. The rows below carry the *reasoning* the generated table cannot: why each check exists and what it cannot see.

**Three stages, not two (2026-07-23).** A handful of load-time rules are pure predicates over the parsed document -- an empty floor, a duplicate floor primitive / config path / task id -- so they are `stage=SYNTAX` `DocumentCheck`s in `checks/document.py`, shared by the loader and the editor. `resolve()` runs them first and raises the first failure exactly as it always did; the editor runs the same four and reports them **all at once, each on its own token**. The rest of the load-time errors deliberately stay in `lang/`: they are raised while *constructing* a library, elaborating a template or applying config, so there is no completed model to be a predicate over. They move when the tolerant resolver gives them one.

**Where the checks live (2026-07-23).** Each check is a `LadderCheck` subclass in `ladders/checks/`, one module per family below: it declares its `code`, `category`, `stage` and `default_severity` in the class body and implements `run(ctx)`. Both families share the `Check` parent (identity + how a finding is stamped) and differ only in what they read. The lint is the explicit ordered tuple `checks/plan.py::CHECK_PLAN` — order is a value, not import order — run by `checks/run.py::lint_spec` over a `CheckContext` whose derivations are all lazy. `LadderSpec.lint()` is a one-line delegation kept for callers. Adding a check is: write the subclass, add an instance to `CHECK_PLAN`. Nothing else — the skipped-checks list, the tier gating and the finding codes all follow from the class body.

**Two input tiers (2026-07-23).** A check is either *structural* — its inputs are the templates, stated solutions, demonstration kinds and config, never the grids — or *corpus-backed*: it reads the generated train/heldout grids, or evaluates a subterm on them. `lint(corpus_backed=False)` runs only the structural tier, which is what lets a *draft* over assumed primitives be linted without an evaluable corpus (`arc-lab lint-ladder --draft`, [cli/lint_ladder.py](../../src/arc_lab/cli/lint_ladder.py)). The ten corpus-backed families — `tasks-exist`, `min-2-train-examples`, `rewrite-shallow`, `distinct-train-inputs`, `outputs-vary`, `not-identity`, `heldout-distinct`, `constant-subterm`, `if-condition-varies`, `mdl-break-even` — are skipped and NAMED in `LadderShape.skipped_checks`, never silently dropped (that list is now DERIVED from the `stage=CORPUS` entries of `CHECK_PLAN`, so it cannot drift from what is actually skipped), so a quiet draft is never read as a sound ladder. The depth sandwich is structural (templates + `depth_limit`), so it runs against the resolved budget; only the grid-dependent checks wait for the corpus. A draft's CLI outcome is always `INCOMPLETE` (exit 2) — its structural findings are shown but advisory; the definite verdict is deferred to a real lint of the finished ladder.
...
- **`ladders/checks/evaluation.py`** (2026-07-22; moved into the `checks/` package 2026-07-23) — the evaluation-backed check CORES, plain functions over plain data (no `LadderSpec`, no `CheckContext`), with an id-memoized bottom-up subterm evaluator. They return `Verdict`s; the owning `LadderCheck` stamps the code.
