...
How the three activities (**SEARCH**, **SEARCH + LEARN**, **STUDY**) are composed from the execution layer's primitives. Sibling to [ARCHITECTURE.md](ARCHITECTURE.md) (the search-engine design); the code lives under `src/arc_lab/program_search/execution/`.
...
3. Record to `runs/<date>/<started_at>_<run_id>/`. _(Recording is owned by the activity; the idempotency check happens before step 1.)_
...
RunRecord                                    # the handle execute() returns; input to analyze_run / study report
  ...
  ├─ artifact paths (runs/<date>/<started_at>_<run_id>/…)   # run_dir is resolved by run_id suffix, not joined directly
...
arc-lab run-study    ──▶  run_study(study_spec)             ──▶  │  N× execute(grid RunSpecs) (cache hits free)
                          ...
                          execute(run_spec) → RunRecord
                            ├─ runs/<date>/<started_at>_<run_id>/results.json present? → cached RunRecord (no execution)
...
Built 2026-07-11 in seven phases (foundations → run data model → scoring/predict → `execute` → activities → study → CLI + docs), with three cross-fork sync points (component serde on the real engines; the `run(train_examples, …)` blindness seam; the `LearnEngine`/`LearnOutcome` port). The full phase plan and per-step notes are in this file's git history (pre-2026-07-13); the locks were re-pinned deliberately in `tests/program_search/execution/test_locks.py` (full numbers + historical interpretation: EXPERIMENT_LOG.md 2026-07-11).

**The old tree was deleted 2026-07-15,** after a dedicated comparison pass (old `dsl/`-tree tests vs the new build: anything missed?) confirmed `src/arc_lab/solvers/` (incl. `dsl/`, `base.py`, `baseline.py`), `eval/runner.py`, `cli_legacy.py`, and the old top-level `tests/test_*.py` (which had guarded the old tree via `tests/test_integration.py`'s locks) were fully superseded — nothing under `program_search/`, `cli/`, or `tests/program_search/` had ever imported them. Three genuine coverage gaps the audit surfaced were ported forward before deletion: the content-addressed library store's round-trip/idempotency tests (`tests/program_search/substrate/test_store.py`), the `bound_var_safe` antiunify soundness regression (`tests/program_search/learn/test_learn_port.py`), and a `render_task` smoke test (`tests/viz/test_render.py`). One file in the old top-level batch, `tests/test_type_aliases.py`, turned out not to be part of the legacy tree at all (a whole-repo lint guard, not a solver test) and was moved to `tests/test_lint_type_aliases.py` instead of being deleted. The eight live-module test files formerly at `tests/test_*.py` had already been moved to `tests/{core,eval,taskgen}/` (2026-07-13) precisely so this deletion couldn't sweep them.
