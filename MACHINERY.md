...
| budget (F1) | value | `Config.budget` (`Budget`: `depth_limit` · `max_arity` · `max_pool`, plus the two **stop limits** `considered_limit` / `solution_limit` and their `*_mode`) — per-run *data*, passed to `run()`, varied across a study grid while the engine stays fixed |
