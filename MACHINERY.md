...
| `BuildGridSearch` (open-term) | bespoke: enumerate coordinate-lambda *bodies* (`$i` seeded), cell-battery obs-equiv dedup, cost-beam, assemble + verify a `build_grid` program | `ABC` | ✅ | **pixels→D4** — re-derives size-general D4 (E5/E7); a hard **beam cliff** (the reflection sits deep in the cost pool) is the concrete "where search breaks" |
...
| Frequent-subtree / e-graph / version-space proposer | DreamCoder-grade invention — **adopt Stitch/babble, don't hand-build** (Stitch: 3–4 orders faster than DreamCoder's version spaces; see MACHINERY-STRATEGY) | `unbuilt` | 🔜 | richer abstractions — **cross-member idioms** (E7: `mirror_index` can't emerge from whole-program antiunification; compression stays ×0.79 without it) |
...
- **The critical path (what gates what) [H].** Cleared. All five mechanisms that gated the downstream have shipped:
  ...
  - **lambda-index binding** (F0) → size-general geometry from cells, the pixels→D4 keystone — _done_ (the Stitch-compatible De Bruijn `$i` / `Lam` / `Closure` substrate + the `build_grid`/`width`/`height`/`sub` clique). And **pixels→D4 is now achieved end-to-end**: `BuildGridSearch` (F1) + the loop re-derive the whole D4 group from the cell floor, blind (E5 rot90; E7 all six), after a bound-var soundness fix to antiunify (E6→E7). What it does *not* yet do is *compress* the ladder — `mirror_index` can't emerge from whole-program antiunification — so the new gate is a **frequent-subtree proposer** (F4).
...
- **Emptiest / highest-leverage families:** F0 is essentially complete and the open-term `build_grid` search shipped (pixels→D4 done), so the frontier the *findings* now point at is a **frequent-subtree proposer** (F4 — to invent cross-member idioms like `mirror_index`; the D4 ladder re-derives but stays uncompressed without it) and the wider A\*/MCTS/stochastic corner — plus, per the strategy, pointing the loop at real ARC tasks rather than another microworld.
