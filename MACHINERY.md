...
| Search-aware / transfer-aware governance | a selector scoring candidates by _usefulness to the search_ (or held-out transfer), not train node-count | `unbuilt` | 🔜 | the general, non-scoping fix — the reusable idiom wins on merit (the divergence's root cause: train-DL ≠ reusability) |
