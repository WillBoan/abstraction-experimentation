...
load exactly like a real dataset) plus a ``manifest.json`` recording each task's ground-truth
label, its split, and a content hash — the recipe that makes a testbed inspectable,
reproducible, and traceable from a run (whose ``dataset`` coordinate is the testbed name).
"""
