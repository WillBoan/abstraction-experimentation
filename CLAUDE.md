...
- **Add a ladder** → one `<name>.ladder` file in `program_search/ladders/registry/` (format spec: `docs/abstraction_ladders/LADDER-FORMAT.md`); there is nothing to register — the directory is scanned. Loop: `arc-lab ladder-seeds --variant N` for paste-ready input grids → `arc-lab lint-ladder <name>` (reads only; works before the testbed exists) → `arc-lab taskgen <name>` to commit the testbed → `run-ladder <name>` to drive it.
