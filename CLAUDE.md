...
When you run a meaningful experiment or reach a finding — **including dead ends** — append a terse, commit-anchored entry to [EXPERIMENT_LOG.md](EXPERIMENT_LOG.md). It is the shared chronological record; read its header before adding to it. [EXPERIMENTS.md](EXPERIMENTS.md) is the rigor-reviewed current synthesis, not an append-only log. The experimental program to date is summarized in [README.md](README.md). There is **no active multi-step experimental plan**; consult the dated [research state](docs/abstraction_ladders/AL-RESEARCH-STATE-2026-08-12.md) before proposing one, and begin resumed work with a new dated, prediction-first plan. Live ladder docs are `docs/abstraction_ladders/` (start at its [README.md](docs/abstraction_ladders/README.md)); what has actually been run is the generated [BATCH-OF-RECORD.md](docs/abstraction_ladders/BATCH-OF-RECORD.md), never prose. [EXPERIMENT_QUEUE.md](docs/EXPERIMENT_QUEUE.md) is the drain-only backlog of non-ladder or deferred experiments.

For a **non-trivial investigation**, also keep a detailed lab notebook under [experiments/](experiments/) — the full write-up plus probe scripts and outputs, which the chronological log points to and the reviewed synthesis may cite. See [experiments/README.md](experiments/README.md).
...
- Commands: `Makefile`
- Current findings: `EXPERIMENTS.md` · Chronological history: `EXPERIMENT_LOG.md` · Detailed notebooks: `experiments/` · Non-ladder backlog: `docs/EXPERIMENT_QUEUE.md`
- **Ladder docs index** (read this first — it states the naming convention): `docs/abstraction_ladders/README.md`
  ...
  - **The register is generated: `BATCH-OF-RECORD.md`** (`arc-lab run-batch`) — per-member verdicts, what the set samples on each axis, which cohorts license which comparisons. There is no hand-maintained register; `LADDERS.md` was deleted 2026-08-03 as covered by this, the `.ladder` files, and the experiment record.
